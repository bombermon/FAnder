class Validator:
    ...
    def checkPartner(self, user, partner):
        ...
        if (partner['id'] != user['id']) and (partner['age'] >= user['p_min_age']) and (partner['age'] <= user['p_max_age']) and ((partner['dialog_status'] == "process") or (partner['dialog_status'] == "freezed")):
